<script lang="ts">
	...
	viewStack.subscribe((arr) => {
		if (!isVisible && arr.includes(uniqueId)) openHandle();
		// sync state.
	...
	const searchHandle = async (e: Record<string, any> | any) => {
		...
		if (preSearchText == searchText) return;
...
<style>
	...
	.search-box .search-bar {
		...
		height: 72px;
		box-shadow: 0 8px 22px 0 rgba(37, 44, 97, 0.15), 0 4px 6px 0 rgba(209, 214, 243, 0.2);
	}
